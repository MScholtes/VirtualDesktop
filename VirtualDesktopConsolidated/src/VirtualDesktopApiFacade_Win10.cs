...
{
    public class VirtualDesktopApiFacade_Win10 : IVirtualDesktopApiFacade
    {
        private readonly IVirtualDesktopManagerInternal_Win10 _managerInternal;
        private readonly IVirtualDesktopManager_Win10 _manager;
        private readonly IApplicationViewCollection_Win10 _viewCollection;
        private readonly IVirtualDesktopPinnedApps_Win10 _pinnedApps;

        public VirtualDesktopApiFacade_Win10()
        {
            ...
            Guid iid = typeof(IVirtualDesktopManagerInternal_Win10).GUID;
            IntPtr ppv;
            int hr = shell.QueryService(ref serviceGuid, ref iid, out ppv);
            if (hr != 0 || ppv == IntPtr.Zero)
                throw new InvalidCastException("QueryService did not return IVirtualDesktopManagerInternal_Win10");
            _managerInternal = (IVirtualDesktopManagerInternal_Win10)Marshal.GetObjectForIUnknown(ppv);
            _manager = (IVirtualDesktopManager_Win10)Activator.CreateInstance(Type.GetTypeFromCLSID(new Guid("AA509086-5CA9-4C25-8F95-589D3C07B48A")));
            iid = typeof(IApplicationViewCollection_Win10).GUID;
            hr = shell.QueryService(ref iid, ref iid, out ppv);
            if (hr != 0 || ppv == IntPtr.Zero)
                throw new InvalidCastException("QueryService did not return IApplicationViewCollection_Win10");
            _viewCollection = (IApplicationViewCollection_Win10)Marshal.GetObjectForIUnknown(ppv);
            serviceGuid = new Guid("B5A399E7-1C87-46B8-88E9-FC5747B171BD");
            iid = typeof(IVirtualDesktopPinnedApps_Win10).GUID;
            hr = shell.QueryService(ref serviceGuid, ref iid, out ppv);
            if (hr != 0 || ppv == IntPtr.Zero)
                throw new InvalidCastException("QueryService did not return IVirtualDesktopPinnedApps_Win10");
            _pinnedApps = (IVirtualDesktopPinnedApps_Win10)Marshal.GetObjectForIUnknown(ppv);
        }
        ...
        public int GetCurrentDesktopIndex()
        {
            ...
            {
                desktops.GetAt(i, typeof(IVirtualDesktop_Win10).GUID, out var obj);
                if (((IVirtualDesktop_Win10)obj).GetId() == current.GetId())
                    return i;
            }
            return -1;
        }
        ...
        public void SwitchDesktop(int index)
        {
            ...
            desktops.GetAt(index, typeof(IVirtualDesktop_Win10).GUID, out var obj);
            _managerInternal.SwitchDesktop((IVirtualDesktop_Win10)obj);
        }
        ...
        public void RemoveDesktop(int index, int fallbackIndex)
        {
            ...
            desktops.GetAt(index, typeof(IVirtualDesktop_Win10).GUID, out var objRemove);
            desktops.GetAt(fallbackIndex, typeof(IVirtualDesktop_Win10).GUID, out var objFallback);
            _managerInternal.RemoveDesktop((IVirtualDesktop_Win10)objRemove, (IVirtualDesktop_Win10)objFallback);
        }
        ...
        public void SetDesktopName(int index, string name) { /* No-op on Win10 */ }
        public bool SupportsWallpaperSetting => false;
        public void SetDesktopWallpaper(int index, string path) => throw new NotSupportedException("Wallpaper setting not supported on Windows 10");

        public void MoveWindowToDesktop(IntPtr hWnd, int desktopIndex)
        {
            if (hWnd == IntPtr.Zero) throw new ArgumentNullException();
            _managerInternal.GetDesktops(out var desktops);
            desktops.GetAt(desktopIndex, typeof(IVirtualDesktop_Win10).GUID, out var objDesktop);
            int processId;
            ...
            if (Process.GetCurrentProcess().Id == processId)
            {
                try
                {
                    _manager.MoveWindowToDesktop(hWnd, ((IVirtualDesktop_Win10)objDesktop).GetId());
                }
                catch
                {
                    _viewCollection.GetViewForHwnd(hWnd, out var view);
                    _managerInternal.MoveViewToDesktop(view, (IVirtualDesktop_Win10)objDesktop);
                }
            }
            else
            {
        ...
        public bool IsWindowOnDesktop(IntPtr hWnd, int desktopIndex)
        {
            ...
            return ((IVirtualDesktop_Win10)objDesktop).GetId() == id;
        }
        ...
        {
            _managerInternal.GetDesktops(out var desktops);
            desktops.GetAt(index, typeof(IVirtualDesktop_Win10).GUID, out var objDesktop);
            int hr = _managerInternal.GetAdjacentDesktop((IVirtualDesktop_Win10)objDesktop, 3, out var leftDesktop); // 3 = LeftDirection
            if (hr == 0)
            {
                for (int i = 0; i < GetDesktopCount(); i++)
                {
                    desktops.GetAt(i, typeof(IVirtualDesktop_Win10).GUID, out var obj);
                    if (((IVirtualDesktop_Win10)obj).GetId() == leftDesktop.GetId())
                        return i;
                }
            }
            return -1;
        ...
        public int GetRightDesktopIndex(int index)
        {
            _managerInternal.GetDesktops(out var desktops);
            desktops.GetAt(index, typeof(IVirtualDesktop_Win10).GUID, out var objDesktop);
            int hr = _managerInternal.GetAdjacentDesktop((IVirtualDesktop_Win10)objDesktop, 4, out var rightDesktop); // 4 = RightDirection
            if (hr == 0)
            {
                for (int i = 0; i < GetDesktopCount(); i++)
                {
                    desktops.GetAt(i, typeof(IVirtualDesktop_Win10).GUID, out var obj);
                    if (((IVirtualDesktop_Win10)obj).GetId() == rightDesktop.GetId())
                        return i;
                }
            }
            return -1;
